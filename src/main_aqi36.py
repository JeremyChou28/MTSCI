...
def main(args):
    ...
    path = "../config/{}.yaml".format(args.dataset)
    with open(path, "r") as f:
        config = yaml.safe_load(f)

    config["model"]["is_unconditional"] = args.unconditional
    config["train"]["beta"]=args.loss_weight
    ...
    saving_path = args.saving_path + "/test/{}".format(dataset)
    if not os.path.exists(saving_path):
        os.makedirs(saving_path)

    print("model folder:", saving_path)

    save_result_path = args.save_result_path + "test/{}".format(dataset)
    ...
    train_loader = generate_train_dataloader(
        dataset_path, seq_len, batch_size=batch_size
    )
    val_loader = generate_val_dataloader(dataset_path, seq_len, batch_size=batch_size)
    test_loader = generate_test_dataloader(dataset_path, seq_len, batch_size=batch_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", default="AQI36", type=str, help="dataset name")
    parser.add_argument(
        "--dataset_path",
        type=str,
        help="parent dir of generated dataset",
        default="../datasets/AQI36/raw_data/",
    )
    parser.add_argument(
        "--save_result_path",
    ...
    parser.add_argument("--seq_len", help="sequence length", type=int, default=36)
    parser.add_argument("--feature", help="feature nums", type=int, default=36)
    parser.add_argument(
        "--checkpoint_path",
        type=str,
