'''
Description: 先对整个数据集模拟制造缺失，然后再滑窗产生样本
...
def generate_val_test_dataloader(dataset_path,
                                 seq_len,
                                 missing_ratio,
    ...
    if missing_pattern == 'block':
        # block missing
        indicating_mask = sample_mask(shape=data.shape,
                                      p=0.0015,
    ...
    print(
        mode +
        ": original missing ratio = {:.4f}, artificial missing ratio = {:.4f}, artificial missing pattern: {}, overall missing ratio = {:.4f}"
        .format(1 - np.sum(gt_mask) / gt_mask.size,
                np.sum(indicating_mask) /
                indicating_mask.size, missing_pattern, 1 -
...
def generate_train_dataloader(dataset_path,
    ...
    if missing_pattern == 'block':
        # block missing
        indicating_mask = sample_mask(shape=train_data.shape,
                                      p=0.0015,
                                      p_noise=missing_ratio,
                                      max_seq=12,
                                      min_seq=12 * 4,
    ...
    mask = gt_mask * (1 - indicating_mask)
    print(
        "Train: original missing ratio = {:.4f}, artificial missing ratio = {:.4f}, artificial missing pattern: {}, overall missing ratio = {:.4f}"
        .format(1 - np.sum(gt_mask) / gt_mask.size,
                np.sum(indicating_mask) / indicating_mask.size,
                missing_pattern, 1 - np.sum(mask) / mask.size))
    ...
    input_X_list,input_mask_list,eval_mask,output_gt_list,pred_gt_list,pred_gt_mask=[],[],[],[],[],[]
...
if __name__ == "__main__":
    ...
    train_loader = generate_train_dataloader(dataset_path,
                                             ...
                                             batch_size=batch_size)
    val_loader = generate_val_test_dataloader(dataset_path,
                                              seq_len,
                                              missing_ratio=miss_rate,
                                              missing_pattern=missing_pattern,
                                              batch_size=batch_size,
                                              mode='val')
    test_loader = generate_val_test_dataloader(dataset_path,
                                               seq_len,
                                               missing_ratio=miss_rate,
                                               missing_pattern=missing_pattern,
                                               batch_size=batch_size,
